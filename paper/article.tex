...
This paper presents the \pkg{evomap} package, a novel toolbox for dynamic mapping in \proglang{Python}. 
Researchers across fields use mapping methods to visualize relationships among objects as spatial representations, or 'maps'.
Most existing statistical software supports only static mapping, which analyzes objects' relationships at a specific point in time. 
They typically lack functionality for dynamic mapping, which analyzes how relationships evolve over time.
The \pkg{evomap} package fills this gap with a Python implementation of the dynamic mapping framework 
EvoMap, initially proposed by \cite{Matthe+Ringel+Skiera:2023}. This framework adapts traditional mapping methods like 
 Multidimensional Scaling for dynamic analysis. In addition to implementing EvoMap, the package includes
 tools for data preprocessing, exploration, and result evaluation, making it a comprehensive toolkit
  for dynamic mapping applications across various disciplines. 
  This paper details the foundations of static and dynamic mapping, describes the architecture and features of \pkg{evomap}, 
and showcases its application with an extensive example. }
...
The package is designed highly modular, facilitating the addition of new mapping methods. Further, it adopts the
popular \pkg{scikit-learn} syntax, ensuring ease of use. 
The package also includes modules for evaluation and exploration of dynamic mapping applications.
...
The installation requires \proglang{Python} version 3.9 or newer, alongside an operational \proglang{C} compiler. 
At the time of writing the latest release of \pkg{evomap} is version 0.3.1. Updates are actively developed on GitHub, 
...
\pkg{evomap} is structured into six key modules\footnote{For clarity, the unified term 'module' encompasses 
...
\code{(9,2)}. A common strategy is to use the output of another (static) mapping method, such as Classical Scaling, for 
...
(i.e., without alignment). The increase in Stress is negligible (+0.0108), which indicates a minimal decrease in static quality 
...
\begin{table}[ht]
  ...
  \begin{tabular}{lp{2.5cm}p{2cm}p{2cm}p{2cm}}
    ...
    \hline
    Independent MDS & 0.9982 & -0.6082 & 0.1802 \\
    Independent MDS \newline + Alignment & 0.3203 & -0.2728 & 0.1802 \\
    EvoMDS & 0.0442 & 0.6405 & 0.1910 \\
    \hline
  \end{tabular}  
...
For instance, all methods necessitate complete observations for each time point. This means that while EvoMap 
