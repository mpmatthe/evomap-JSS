...
At the time of writing the latest release of \pkg{evomap} is version 0.4.4. Updates are actively developed on GitHub, 
...
\pkg{evomap} relies on a limited number of other packages. It uses \pkg{numpy} and \pkg{scipy} for numerical 
computations, and \pkg{matplotlib} and \pkg{seaborn} for plotting. Additionally, \pkg{evomap} uses individual 
routines from \pkg{scikit-learn} and \pkg{statsmodels}, and employs the just-in-time \proglang{C}-compiler \pkg{numba} 
to optimize runtime.
...
Note that the results reported in this paper were generated using Python version 3.12.0 and \pkg{evomap} 
version 0.4.4 on macOS 13.5.2. While the \pkg{evomap} package is designed to be platform-independent, 
